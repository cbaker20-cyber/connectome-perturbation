...
There is code in `perturbation/perturb.py` that writes a file named `results/perturbation_summary.csv`. However, the tracked summary is not bound to a run manifest, configuration, log, input checksums, or commit. Perturbation scripts now resolve connectome inputs through `data/input_manifest.json` via `tools/path_resolver.py`, with a legacy `Drosophila_brain_model/` fallback for older checkouts.
