...
def ensure_repo_on_path(start: Path | None = None) -> Path:
    """Insert absolute repo and ``perturbation/`` paths on ``sys.path``.

    Replaces legacy relative ``sys.path.insert`` hacks so scripts remain
    importable when invoked from any working directory.
    """
    root = repo_root_from(start)
    for candidate in (root, root / "perturbation"):
