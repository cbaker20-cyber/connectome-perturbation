...
def main():
    dest=ROOT/'results/pcdr/composition_audit_20260922/motor_witness'; dest.mkdir(exist_ok=False)
    atomic_json(dest/'protocol.json',{'recorded_utc':now(), 'purpose':'Post-pilot motor-composition matching feasibility only; no simulation or automatic null inference. Five 30second solves maximum. Failure is not proof original pooled-SMD feasible region empty.',
        'method':'Binary integer selection from union of nearest-64 neighborhoods rebuilt inside exact motor/sign/recruitment strata; exact size/sign/recruitment/motor counts.',
        ...
        'objective':'Five fixed independent uniform random linear objectives from seed 631100; exclude previous whole sets with no-good constraints.',
    ...
    rng=np.random.default_rng(631100); records=[];members=[];seen=[]
    for run in range(5):
