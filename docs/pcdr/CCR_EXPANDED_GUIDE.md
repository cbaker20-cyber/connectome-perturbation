...
Installation update, 25 September: James confirmed that pip installation is available in the lab's Jupyter session. The setup cell installs into `.ccr-venv` using that environment's Python and checks both dependency versions and actual imports. It does not replace the notebook kernel's shared packages. Leave `FORCE_REINSTALL = False` normally; set it to True if this dedicated environment needs repair. An optional `BASE_PYTHON` path can be entered directly in the setup cell. Use a CCR module-provided Python 3.11; CCR documents `module load gcc python` for terminal sessions. If no compatible interpreter is available in the session, select the appropriate module before launching Jupyter. See [CCR Python guidance](https://docs.ccr.buffalo.edu/en/latest/howto/python/). Installing packages successfully and executing this particular model are separate checks, so run the four smoke trials before the main study.
