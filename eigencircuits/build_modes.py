...
from .common import atomic_json, atomic_parquet, now, provenance, read_json, sha256, sugar_ids
...
def build(out, baseline_features, seed=630300):
    out = Path(out)
    ...
    if sum(x[2] for x in unique_modes(vals)) < 20:
        k = 80
        vals, vecs, residuals = solve(w, k, seed)
    ...
    for rank, (i, partner, complete) in enumerate(unique_modes(vals)):
        ...
        if complete and row["stable"]:
            stable_rank += 1
        row["eligible"] = bool(row["stable_mode_rank"] is not None and row["stable_mode_rank"] < 20 and eligible_n >= 10
    ...
    atomic_json(out / "selection.json", {"mode_rank": selected, "status": "selected" if candidates else "no_eligible_mode",
                ...
                "selection_rule": "greatest recruited power among first 20 stable complete modes; magnitude breaks ties"})
    atomic_json(out / "manifest.json", {"created_utc": now(), "k": k, "seeds": [seed, seed+1],
                ...
                "claim_status": "structural_hypothesis_only"})
