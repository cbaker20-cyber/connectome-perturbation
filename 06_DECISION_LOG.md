...
| D008 | 2026-07-21 | Prefer conda `brian2` env for overnight JO/BORA jobs; keep pip `requirements.txt` as a secondary install path; pin `Cython<3`. | System `python3` lacked pandas/brian2; Cython 3.x breaks Brian2 2.5.1 codegen (`get_cython_cache_dir`). | Overnight jobs must prepend `$HOME/miniconda3/envs/brian2/bin` to `PATH`; Cython pin avoids silent numpy fallback. | If a dedicated venv or lockfile replaces conda, or Brian2 is upgraded past 2.5.1. |
